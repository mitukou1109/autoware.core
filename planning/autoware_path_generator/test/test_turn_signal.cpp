...
TEST_P(GetTurnSignalTest, getTurnSignal)
{
  ...
  const auto result = utils::get_turn_signal(
    ...
    p.resampling_interval, p.angle_threshold_deg, p.base_link_to_front);
...
TEST_F(UtilsTest, getTurnSignalRequiredEndPoint)
{
  ...
  constexpr double resampling_interval = 1.0;
  constexpr double angle_threshold_deg = 15.0;

  const auto result = utils::get_turn_signal_required_end_point(
    planner_data_.lanelet_map_ptr->laneletLayer.get(lane_id), resampling_interval,
    angle_threshold_deg);

  // Reference:
  // autoware::behavior_path_planner::TurnSignalDecider::get_required_end_point()
  ASSERT_NEAR(result.x(), 3761.616, 1e-3);
  ASSERT_NEAR(result.y(), 73748.634, 1e-3);
}
}  // namespace autoware::path_generator